...
import { recordMilestoneAttempt } from "@/lib/milestones/actions";
import { PASS_SCORE, type MilestoneHalf, type MilestoneQuestion } from "@/lib/milestones/select";
...
export default function MilestoneQuiz({
  ...
  async function handleNext() {
    ...
    setSaving(true);
    const result = await recordMilestoneAttempt(hskLevel, half, score, total);
    setSaving(false);
  ...
  return (
    <main style={{ maxWidth: 480, margin: "0 auto", padding: "16px 16px 40px" }}>
      ...
      >
        <p style={{ fontSize: 40.8, fontWeight: 700, color: "var(--ink)", marginBottom: 20 }}>
          {current.hanzi}
        </p>
